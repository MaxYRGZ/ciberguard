...
export default class LocalDB {
  ...
  static async init() {
    ...
    db.transaction((tx) => {
      ...
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS chatgpt_responses (id INTEGER PRIMARY KEY AUTOINCREMENT, question TEXT NOT NULL, response TEXT NOT NULL, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)',
        [],
        () => console.log('Created table ChatGPT Responses'),
        (error) => console.error('Error creating table ChatGPT Responses:', error)
      );
